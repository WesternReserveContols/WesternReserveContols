...
BYTE RecvChar(void)
{
   ...
   {
      if(Ascii_attrib.Framing == FRAME_8N1);         //special case of 8N1
      else if(Ascii_attrib.Parity == NONE) chr &= 0x7F;         //no parity
      else
      {
...
//            g_status = PARITY_ERROR;     //if parity bits not equal
   ...
   {
      if(Ascii_attrib.Parity == NONE)  //no parity
      {
         ;
      }
      else
